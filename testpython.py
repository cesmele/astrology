...
class User:
  def __init__(self):
    ...
    birthday = input("Enter your birthday in the following format MM/DD/YYYY: ")
    if len(birthday) != 10:
    ...
    if int(birthday[0:2]) < 0 or int(birthday[0:2]) > 12:
    ...
    if month == 11 and day > 30: 
      ...
      self.age = age_convert(int(year), month, day)
      self.zodiac = zodiac_convert(month, day)
      self.chinese_zodiac = chinese_zodiac_convert(year)
      self.matches = []    
    ...
    def __init__(self, path):
        ...
        with open(path, "r", encoding = "utf-8") as f:
            for line in f:
                ...
                age = age_convert(year, month, day)
                matches = []
                inbox = []
...
def match(user, dictionary):
  ...
  if len(full_match) == 0:
    ...
    #print(get_email(name,full_match))
...
def send_message(user, name, email, catalog):
  ...
  for name in catalog:
    if email == catalog[name][3]:
      catalog[name][5].append(f"New request from {user.name}: {message}")
